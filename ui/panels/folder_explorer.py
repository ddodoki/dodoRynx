...
@functools.lru_cache(maxsize=1)   # 최초 1회만 실행, 이후 캐시 반환
...
def invalidate_ext_cache() -> None:
    """SUPPORTED_EXTENSIONS 런타임 변경 시 호출."""
    _get_supported_image_exts.cache_clear()
...
class _BranchDelegate(QStyledItemDelegate):
    ...
    # ── 클래스 수준 상수로 이동 (인스턴스 생성 비용 0) ─────────────────────
    _ARROW_FONT = QFont()
    _ARROW_FONT.setPointSize(7)

    _BADGE_FONT = QFont()
    _BADGE_FONT.setPointSize(7)
    _BADGE_FONT.setBold(True)
    # ────────────────────────────────────────────────────────────────────────

    def __init__(self, empty_set: set[str], parent=None) -> None:
        super().__init__(parent)
        self._empty: set[str] = empty_set
...
class FolderExplorer(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self._fav_tree.path_activated.connect(self._on_path_activated)

        root.addWidget(self._fav_tree, 1)   
    ...
    def _set_tab(self, tab: str) -> None:
        """'normal' 또는 'fav' 탭 전환."""
        is_fav = (tab == "fav")
        self._normal_tree.setVisible(not is_fav)
        self._fav_tree.setVisible(is_fav)
    ...
    def _create_folder(self, checked: bool = False, parent_path: Optional[Path] = None) -> None:
        ...
        if not base or not base.is_dir():
            return
        dlg = _DarkInputDialog(
            self,
            title=t("folder_explorer.dialog.new_folder_title"),
            label=t("folder_explorer.dialog.new_folder_label"),
        )
        if dlg.exec() != QDialog.DialogCode.Accepted:
            return
        new_path = base / dlg.value()
        try:
            ...
            _DarkMessageBox(
                self,
                kind="warning",
                title=t("folder_explorer.dialog.error_title"),
                body=t("folder_explorer.dialog.new_folder_fail") + str(e),
            ).exec()
    ...
    def _rename_folder(self, path: Path) -> None:
        dlg = _DarkInputDialog(
            self,
            title=t("folder_explorer.dialog.rename_title"),
            label=t("folder_explorer.dialog.rename_label"),
            text=path.name,
        )
        if dlg.exec() != QDialog.DialogCode.Accepted:
            return
        new_path = path.parent / dlg.value()
        try:
            ...
            _DarkMessageBox(
                ...
                kind="warning",
                title=t("folder_explorer.dialog.error_title"),
                body=t("folder_explorer.dialog.rename_fail") + str(e),
            ).exec()
    ...
    def _paste_folder(self, dst: Optional[Path]) -> None:
        ...
        if not target_dir or not target_dir.is_dir():
            _DarkMessageBox(
                self,
                kind="warning",
                title=t("folder_explorer.dialog.paste_title"),
                body=t("folder_explorer.dialog.paste_no_target"),
            ).exec()
            return
        ...
        if self._clipboard_paths:
            ...
            if not mime.hasUrls():
                _DarkMessageBox(
                    ...
                    kind="info",
                    title=t("folder_explorer.dialog.paste_title"),
                    body=t("folder_explorer.dialog.paste_no_clip"),
                ).exec()
                return
            sources = [
        ...
        if failed:
            ...
            _DarkMessageBox(
                ...
                kind="warning",
                title=t("folder_explorer.dialog.error_title"),
                body=t("folder_explorer.dialog.paste_fail") + "\n" + msg,
            ).exec()
    ...
    def _delete_folder(self, path: Path) -> None:
        _confirm = _DarkMessageBox(
            self,
            kind="question",
            title=t("folder_explorer.dialog.delete_title"),
            body=t("folder_explorer.dialog.delete_confirm", name=path.name),
        )
        ...
        try:
            ...
            _DarkMessageBox(
                ...
                kind="danger",
                title=t("folder_explorer.dialog.error_title"),
                body=t("folder_explorer.dialog.delete_fail") + str(e),
            ).exec()
