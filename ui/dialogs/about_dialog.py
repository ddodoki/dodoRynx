...
class AboutDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.setStyleSheet("""
            ...
            QPushButton:default { background-color: #0d7dd9; border-color: #0d7dd9; }
            QPushButton:default:hover { background-color: #1088eb; }
            QScrollBar:vertical { background-color: #1a1a1a; width: 10px; border-radius: 5px; }
            QScrollBar::handle:vertical { background-color: #3d3d3d; border-radius: 5px; min-height: 30px; }
    ...
    def _init_ui(self):
        ...
        header = QWidget()
        header.setStyleSheet("QWidget { background-color: #0d7dd9; }")
        ...
        intro_text.setStyleSheet("""
            padding: 12px; background-color: #2b2b2b;
            border: 1px solid #3d3d3d; border-radius: 6px; line-height: 1.5;
        """)
    ...
    def _open_licenses_folder(self):
        ...
        if not licenses_dir.exists():
            _DarkMessageBox(
                self, kind='warning',
                title=t('about_dialog.no_folder_title'),
                body=t('about_dialog.no_folder_msg', path=licenses_dir),
            ).exec()
            return
        ...
        if not ok:
            _DarkMessageBox(
                self, kind='warning',
                title=t('about_dialog.open_error_title'),
                body=t('about_dialog.open_error_msg'),
            ).exec()
