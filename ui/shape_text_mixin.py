...
class ShapeTextMixin:
    ...
    def _show_shape_text_overlay(self) -> None:
        ...
        for row_shapes in (_SHAPES[:6], _SHAPES[6:]):
            ...
            for i, (key, icon, tip_key) in enumerate(row_shapes):
                ...
                btn.setToolTip(t(tip_key))          # ← 번역 키로 변경
        ...
        btn_bold.setStyleSheet("font-weight:700;")
    ...
    def _st_sync_from_text_item(self, item: TextShapeItem) -> None:
        ...
        self._st_font_size   = self._st_to_viewport_size(item._font_size)
    ...
    def _st_show_text_edit_dialog(self, item: TextShapeItem) -> None:
        from PySide6.QtWidgets import (
            ...
            QHBoxLayout, QLabel, QTextEdit, QVBoxLayout,
        )
        parent_w = self.window()  # type: ignore[attr-defined]
        dlg = QDialog(parent_w)
        dlg.setWindowTitle(t('shape_text_mixin.text_edit_title'))
        ...
        row = QHBoxLayout()
        font_cb = QFontComboBox()
        font_cb.setCurrentFont(QFont(item._font_family))

        fs_spin = QSpinBox()
        fs_spin.setRange(8, 500)
        fs_spin.setValue(item._font_size)
        fs_spin.setSuffix(" px")
        ...
        btn_bold.setChecked(item._bold)
        btn_italic.setChecked(item._italic)
        btn_bold.setStyleSheet("font-weight:700;")
        btn_italic.setStyleSheet("font-style:italic;")
        ...
        def _refresh_col():
            lum = color[0].lightness()
            btn_color.setStyleSheet(
                f"background:{color[0].name()};"
        ...
        btn_color.clicked.connect(
            lambda: [
                color.__setitem__(0, c) or _refresh_col()
                for c in [QColorDialog.getColor(color[0], dlg)]
                if c.isValid()
            ]
        )

        for w in (font_cb, fs_spin, btn_bold, btn_italic, btn_color):
            row.addWidget(w)
        layout.addLayout(row)

        layout.addWidget(QLabel(t('shape_text_mixin.text_label')))
        text_edit = QTextEdit()
        text_edit.setPlainText(item._text)
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted:
            ...
            item.update_properties(
                text = text_edit.toPlainText() or t('shape_text_mixin.default_text'),
