...
class ModelDownloadDialog(QDialog):
    ...
    worker : progress(int,int) / finished() / failed(str) 시그널을 가진 QThread.
             ModelDownloadWorker(ai_bg_remover) 또는
             AIModelDownloadWorker(ai_model_manager) 모두 주입 가능.
    title  : 다이얼로그 제목
    desc   : 설명 텍스트
    filename: 하단 파일명 표시 (생략 가능)
    """
    ...
    def __init__(
        ...
        self._worker.finished.connect(self._on_finished)   # type: ignore[union-attr]
        self._worker.failed.connect(self._on_failed)       # type: ignore[union-attr]
    ...
    def _on_progress(self, downloaded: int, total: int) -> None:
        ...
        self._bar.setValue(min(99, pct))  # finished 시그널에서 100으로 마무리
    ...
    def _on_failed(self, error: str) -> None:
        ...
        QMessageBox.critical(
            self,
            t('bg_remove.dl_fail_title'),
            t('bg_remove.dl_fail_msg', error=error[:400]),
        )
        self.reject()
    ...
    def _on_cancel(self) -> None:
        ...
        if self._worker.isRunning():
            if hasattr(self._worker, 'cancel'):
                self._worker.cancel() 
            try:
                self._worker.finished.disconnect(self._on_finished)  # type: ignore[union-attr]
                self._worker.failed.disconnect(self._on_failed)       # type: ignore[union-attr]
            except RuntimeError:
                pass
            self._worker.finished.connect(self.reject)               # type: ignore[union-attr]
            self._worker.failed.connect(lambda _: self.reject())     # type: ignore[union-attr]
        else:
            self.reject()
