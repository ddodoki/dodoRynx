...
from core.map_loader import read_pmtiles_header, validate_pmtiles
...
class SettingsDialog(QDialog):
    ...
    def _create_map_tab(self) -> QWidget:
        """지도 설정 탭 — PMTiles 파일 경로 + 최대 줌"""
        widget = QWidget()
        layout = QVBoxLayout(widget)
        layout.setSpacing(10)

        # ── PMTiles 파일 그룹 ────────────────────────────────────────────────
        file_group  = QGroupBox(t('settings.map.pmtiles_group'))
        file_layout = QVBoxLayout(file_group)
        file_layout.setSpacing(8)

        # 안내
        guide = QLabel(t('settings.map.pmtiles_guide'))
        ...
        """)
        file_layout.addWidget(guide)

        # 경로 입력
        path_row = QHBoxLayout()
        self.pmtiles_path_edit = QLineEdit()
        self.pmtiles_path_edit.setPlaceholderText(t('settings.map.pmtiles_placeholder'))
        self.pmtiles_path_edit.setReadOnly(True)
        self.pmtiles_path_edit.setStyleSheet("""
            QLineEdit {
        ...
        """)
        path_row.addWidget(self.pmtiles_path_edit, 1)

        browse_btn = QPushButton("📂 " + t('settings.map.pmtiles_browse'))
        ...
        """)
        browse_btn.clicked.connect(self._browse_pmtiles)
        path_row.addWidget(browse_btn)

        clear_path_btn = QPushButton("✕")
        clear_path_btn.setFixedWidth(30)
        clear_path_btn.setFixedHeight(30)
        clear_path_btn.setToolTip(t('settings.map.pmtiles_clear_tooltip'))
        clear_path_btn.setStyleSheet("""
            QPushButton {
                background: #4a2a2a; color: #ff8888;
                border: none; border-radius: 4px; font-weight: bold;
            }
            QPushButton:hover { background: #5a3a3a; }
        """)
        clear_path_btn.clicked.connect(self._clear_pmtiles_path)
        path_row.addWidget(clear_path_btn)

        file_layout.addLayout(path_row)

        self.pmtiles_info_label = QLabel(t('settings.map.pmtiles_no_file'))
        self.pmtiles_info_label.setWordWrap(True)
        self.pmtiles_info_label.setStyleSheet("""
            QLabel {
        ...
        """)
        file_layout.addWidget(self.pmtiles_info_label)
        ...
        self.max_zoom_spin.setRange(1, 22)
        self.max_zoom_spin.setValue(14)
        self.max_zoom_spin.setToolTip(t('settings.map.max_zoom_tooltip'))
        zoom_form.addRow(t('settings.map.max_zoom_label'), self.max_zoom_spin)
    ...
    def _browse_pmtiles(self) -> None:
        """PMTiles 파일 선택 다이얼로그"""
        start_dir = ""
        current = self.pmtiles_path_edit.text().strip()
        if current:
            start_dir = str(Path(current).parent)

        file_path, _ = QFileDialog.getOpenFileName(
            self,
            t('settings.map.pmtiles_select_title'),
            start_dir,
            t('settings.map.pmtiles_file_filter'),
        )
        if file_path:
            self.pmtiles_path_edit.setText(file_path)
            self._update_pmtiles_info(Path(file_path))


    def _clear_pmtiles_path(self) -> None:
        self.pmtiles_path_edit.clear()
        self.pmtiles_info_label.setText(t('settings.map.pmtiles_no_file'))
        self.pmtiles_info_label.setStyleSheet("""
            QLabel {
                color: #888; font-size: 11px;
                background: rgba(80,80,80,30);
    ...
    def _update_pmtiles_info(self, path: Path) -> None:
        """PMTiles 헤더를 읽어 파일 정보 레이블 갱신."""
        valid, reason = validate_pmtiles(path)
        if not valid:
            self.pmtiles_info_label.setText(f"❌  {reason}")
            self.pmtiles_info_label.setStyleSheet("""
                QLabel {
        ...
        if header is None:
            self.pmtiles_info_label.setText(t('settings.map.header_parse_fail'))
            return

        size_mb   = path.stat().st_size / 1024 / 1024
        tile_type = {
            1: t('settings.map.tile_type_mvt'),
            2: t('settings.map.tile_type_png'),
            3: t('settings.map.tile_type_jpeg'),
            4: t('settings.map.tile_type_webp'),
        }.get(
            header['tile_type'],
            t('settings.map.tile_type_unknown', n=header['tile_type'])
        )

        info_text = t(
            'settings.map.pmtiles_info_text',
            tile_type=tile_type,
            min_zoom=header['min_zoom'],
            max_zoom=header['max_zoom'],
            size_mb=f"{size_mb:.1f}",
        )
        self.pmtiles_info_label.setText(info_text)
        self.pmtiles_info_label.setStyleSheet("""
            QLabel {
        ...
        # 최대 줌 자동 설정
        self.max_zoom_spin.setValue(header['max_zoom'])
    ...
    def _clear_render_cache(self) -> None:
        ...
        reply = QMessageBox.question(
            self,
            t('dialog.render_cache_clear_title'),
            t('dialog.render_cache_clear_msg'),
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No,
        )
        if reply != QMessageBox.StandardButton.Yes:
            return
        # 모듈 참조로 항상 최신 _render_cache 객체 접근
        _map_loader_module._render_cache.clear()
        QMessageBox.information(
            self,
            t('dialog.render_cache_cleared_title'),
            t('dialog.render_cache_cleared_msg'),
        )
    ...
    def _load_settings(self) -> None:
        ...
        # 지도 (PMTiles)
        pmtiles_path = self.config.get('map.pmtiles_path', '')
        self.pmtiles_path_edit.setText(pmtiles_path)
        if pmtiles_path:
            self._update_pmtiles_info(Path(pmtiles_path))

        self.max_zoom_spin.setValue(self.config.get('map.max_zoom', 14))
    ...
    def accept(self) -> None:
        ...
        # PMTiles 설정
        new_pmtiles_path = self.pmtiles_path_edit.text().strip()
        new_max_zoom     = self.max_zoom_spin.value()
        old_pmtiles_path = self.config.get('map.pmtiles_path', '')
        old_max_zoom     = self.config.get('map.max_zoom', 14)
        _map_changed = False

        if new_pmtiles_path != old_pmtiles_path:
            # 경로가 설정된 경우 유효성 검증
            if new_pmtiles_path:
                valid, reason = validate_pmtiles(Path(new_pmtiles_path))
                if not valid:
                    QMessageBox.warning(
                        self,
                        t('settings.map.invalid_pmtiles_title'),
                        f"{t('settings.map.invalid_pmtiles_msg')}\n{reason}",
                    )
                    return   # 저장 중단, 다이얼로그 유지
            self.config.set('map.pmtiles_path', new_pmtiles_path)
            _map_changed = True

        if new_max_zoom != old_max_zoom:
            self.config.set('map.max_zoom', new_max_zoom)
            _map_changed = True
    ...
    def _delete_cache_dir(self, cache_dir: Path) -> int:
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                t('dialog.cache_delete_error_title'),
                t('dialog.cache_delete_error_msg', error=e),
            )
