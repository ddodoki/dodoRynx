# -*- coding: utf-8 -*-
# tools\printing\print_dialog.py
...
from PySide6.QtCore import QMarginsF, Qt, Signal, QThread
...
class PrintDialog(QDialog):
    ...
    def _on_printer_properties(self):
        ...
        try:
            ...
            if not printer_name:
                _DarkMessageBox(self, kind='warning', title=t('print.no_printer_title'), body=t('print.no_printer_msg')).exec()
                return
            
            _DarkMessageBox(
                self, kind='info',
                title=t('print.props_title'),
                body=t('print.props_msg', name=printer_name),
            ).exec()
        
        except ImportError:
            _DarkMessageBox(self, kind='info', title=t('print.props_info_title'), body=t('print.props_info_msg')).exec()
    ...
    def _on_save_pdf(self):
        ...
        try:
            ...
            _DarkMessageBox(self, kind='danger', title=t('print.error_title'), body=t('print.pdf_error_msg', error=e)).exec()
    ...
    def _on_print(self):
        ...
        try:
            ...
            printer_name = self.printer_combo.currentText()
            if not printer_name:
                _DarkMessageBox(self, kind='danger', title=t('print.error_title'), body=t('print.no_printer_msg')).exec()
                return
            ...
            if is_pdf_printer:
                ...
                _pdf_dlg = _DarkMessageBox(
                    self, kind='question',
                    title=t('print.pdf_printer_title'),
                    body=t('print.pdf_printer_msg', name=printer_name),
                )
                
                if _pdf_dlg.exec() == QDialog.DialogCode.Accepted:
                    self._on_save_pdf()
                    return
                else:
            ...
            _confirm = _DarkMessageBox(
                self, kind='question',
                title=t('print.confirm_title'),
                body=t('print.confirm_msg',
            ...
            if not printer:
                _DarkMessageBox(self, kind='danger', title=t('print.error_title'), body=t('print.create_fail_msg')).exec()
                return
            ...
            error_print(f"인쇄 실패: {e}")
            error_print(traceback.format_exc())
            _DarkMessageBox(self, kind='danger', title=t('print.error_title'), body=t('print.print_error_msg', error=e)).exec()
    ...
    def _execute_print_thread(self, printer: QPrinter, settings: Dict[str, Any], is_pdf: bool = False):
        ...
        if self.print_thread is not None and self.print_thread.isRunning():
            _DarkMessageBox(
                self, kind='warning',
                title=t('print.warn_title'),
                body=t('print.already_printing'),
            ).exec()
            return
        ...
        progress.setStyleSheet("""
            ...
            QProgressBar {
                ...
                border-radius: 4px; text-align: center; color: #cccccc;
            }
        ...
        def on_completed():
            ...
            if is_pdf:
                ...
                _DarkMessageBox(self, kind='info', title=t('print.done_title'), body=t('print.done_msg')).exec()
                self.accept()
        
        def on_failed(error):
            progress.close()
            msg_key = 'print.print_fail_msg' if not is_pdf else 'print.pdf_fail_msg'
            _DarkMessageBox(self, kind='danger', title=t('print.error_title'), body=t(msg_key, error=error)).exec()
    ...
    def _show_pdf_complete_dialog(self):
        ...
        def open_folder():
            ...
            try:
                ...
                _DarkMessageBox(dialog, kind='warning', title=t('print.warn_title'), body=t('print.folder_open_fail', error=e)).exec()
        ...
        def open_file():
            ...
            try:
                ...
                _DarkMessageBox(dialog, kind='warning', title=t('print.warn_title'), body=t('print.pdf_open_fail', error=e)).exec()
        ...
        try:
            ...
            if self.print_thread and self.print_thread.isRunning():
                _close_dlg = _DarkMessageBox(
                    self, kind='question',
                    title=t('print.in_progress_title'),
                    body=t('print.in_progress_msg'),
                )
